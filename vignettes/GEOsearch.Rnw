...
\uGEOsearchckage[utf8]{inputenc}
\uGEOsearchckage{textcomp}
...
\section{GEOsearch GUI}
In addition to the basic command lines tools discussed above, GEOsearch provides a powerful which provides more comprehensive and convenient functions for gene expression pattern analysis. For example, users can easily transform raw gene expression data and convert gene identifiers before the analysis; save the result tables and plots of publication quality; identify genes with different expression patterns on true time and pseudo-time axis. Users are encouraged to use GEOsearch GUI.
